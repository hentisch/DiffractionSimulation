...
\begin{frame}{Crystall Lattices}

	\begin{columns}
	
	\begin{column}{.5\textwidth}
		In a crystal structure, atoms are organized into a grid
		\begin{itemize}
			\item The most basic repreating unit in a crystal structure is known as it's unit cell
			\item This unit cell is then repreated to form an entire crystall
			\item Note that in most real crystalls, the lattice is not perfect as is shown in these diagrams
		\end{itemize}
	\end{column}	
	
	\begin{column}{.5\textwidth}
	\begin{figure}
		\animategraphics[autoplay,loop,width=0.5\textwidth]{4}{simple_cubic_lattice/frame-}{0}{99}	
