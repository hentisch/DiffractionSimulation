...
def get_electric_scalar_field(size:tuple, wavevector_orgin:tuple, time:float, wave_amplitude:float):
    ...
    for x in range(size[0]):
        for y in range(size[1]):
            for z in range(size[2]):
                cloud[x, y, z] = ts.magnitude_by_space(atom_point, (x, y, z), wavevector_orgin, time, wave_amplitude)
...
class FieldModel(HasTraits):
    ...
    def __init__(self) -> None:
        ...
        self.s = mlab.pipeline.volume(mlab.pipeline.scalar_field(np.zeros((20, 20, 20)), figure=self.scene.mayavi_scene))
    
    @on_trait_change('slider')
    def slider_changed(self):
        self.s.mlab_source.scalars = get_electric_scalar_field((20, 20, 20), (0, 10, 10), self.slider/10, 1)
