...
def plot_cylinder_line(point_a:tuple, point_b:tuple, scale_factor:float, resolution:float, figure:mlab.figure):
    ...
    for i, point in enumerate(all_points[:-1]):
        mlab.quiver3d(*point, *all_points[i+1], mode="arrow", resolution=resolution, color=rgb_to_mayavi(0, 0, 255), figure=figure)

def main(image_path=None, size=(500, 500)):
    ...
    mlab.plot3d(magnetic_field[:,0], magnetic_field[:,1], magnetic_field[:,2], color=rgb_to_mayavi(255, 0, 0), figure=fig)
    ax = mlab.axes(nb_labels = 1, xlabel="Strength of magnetic field", x_axis_visibility=False, y_axis_visibility=False, figure=fig)
    ax.axes.font_factor = 1

    mlab.plot3d(electric_field[:,0], electric_field[:,1], electric_field[:,2], color=rgb_to_mayavi(255, 255, 0), figure=fig)

    ranges=(endpoints[0][0], endpoints[1][0], endpoints[0][1], endpoints[1][1], -1, 1)
    ax = mlab.axes(nb_labels = 1, ranges=ranges, zlabel="Strength of Electric Field", ylabel="Distance", figure=fig)
    ax.axes.font_factor = 1

    if image_path != None:
        shot = mlab.screenshot(fig, 'rgb', True)
...
if __name__ == "__main__":
    main(image_path="/home/henry/DiffractionSimulation/pic.png", size=(1000, 1000))